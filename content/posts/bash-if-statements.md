...
## `-eq` 
`-eq` is used to compare integers. beside `-eq` there are a other operators for comparing integers. There are These operators are defined by POSIX. The list below shows these operators
...
It is is also possible to compare integers by using `<`, `>` and `==` using arithmatic evaluation. Within an arithmetic evaluation arithmetic can be done using the same syntax as in C. This emans the `<` and `>` can be used for comparing numbers and won't work as a redirect.
